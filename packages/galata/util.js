...
const configKeys = {
    "string": new Set(["browserType", "browserUrl", "jlabBaseUrl", "jlabToken", "testId", "testOutputDir", "referenceDir"]),
            ...
            switch (keyValueType) {
                ...
                case "custom":
                    switch (key) {
                        case "include":
                        case "exclude":
                            const suites = JSON.parse(value);
                            config[key] = suites.filter(suite => suite !== '');
                    }
                    break;
            }
        } catch (error) {
