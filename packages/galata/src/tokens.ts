...
export type CaptureType = 'image' | 'html';
...
export type TestCaptures = { [key: string]: { [key: string]: ICapture[] } };

export type TestLogs = { [key: string]: { [key: string]: ITestLog[] } };

export interface ICreateNewPageOptions {
  generateWorkspace?: boolean;
  onPageCreated?: (page: Page) => Promise<void> | void;
  onPageLoaded?: (page: Page) => Promise<void> | void;
}

export type CreateNewPageFunction = (
  options?: ICreateNewPageOptions
) => Promise<Page>;

export interface IGalataContext {
  testId: string;
  browserType: string;
  jlabBaseUrl: string;
  jlabToken: string;
