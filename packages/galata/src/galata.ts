...
import {
  ...
  ICapture,
  ITestLog,
  ICreateNewPageOptions
} from './tokens';

let _runCallbacksExposed = 0;
let _currentSuite: string;
let _currentTest: string;

function describeWrapper(
...
export namespace galata {
  ...
  async function _getElementClassList(
  ...
  export const context: IGalataContext = global.__TEST_CONTEXT__;
  ...
  export function xpBuildActivityTabSelector(name: string): string {
    return `//div[${xpContainsClass('jp-Activity')}]/ul/li[${xpContainsClass(
      'lm-TabBar-tab'
    )} and ./div[text()="${name}" and ${xpContainsClass(
      'lm-TabBar-tabLabel'
    )}]]`;
  }
  ...
  function logTestCapture(capture: ICapture) {
    ...
    for (let c = 0; c < numCaptures; ++c) {
      ...
      if (
        existingCapture.name === capture.name &&
        capture.type === existingCapture.type
      ) {
  ...
  export function log(log: ITestLog): void {
  ...
  export namespace capture {
    ...
    export async function compareScreenshot(
      ...
      const referenceImage = PNG.sync.read(
        fs.readFileSync(`${context.referenceDir}/${referencePath}`)
      );
      const testImage = PNG.sync.read(
        fs.readFileSync(`${context.testOutputDir}/${filePath}`)
      );
      ...
      if (diff > 0) {
        ...
        fs.writeFileSync(
          `${context.testOutputDir}/${diffPath}`,
    ...
    export async function compareHTML(
      ...
      const referenceContent = fs.readFileSync(
        `${context.referenceDir}/${referencePath}`
      );
      const testContent = fs.readFileSync(
        `${context.testOutputDir}/${filePath}`
      );
  ...
  export namespace kernel {
    export async function isAnyRunning(): Promise<boolean> {
      return await context.page.evaluate(() => {
        return (
          window.jupyterlab.serviceManager.sessions.running().next() !==
          undefined
        );
      });
    }
  ...
  export namespace menu {
    ...
    export async function getMenuBarItem(
      label: string
    ): Promise<ElementHandle<Element> | null> {
      return await context.page.$(
        `xpath=//li[./div[text()="${label}" and ${xpContainsClass(
    ...
    export async function getMenuItem(
      ...
      for (let i = 0; i < numParts; ++i) {
        ...
        const menuItem =
          i === 0
            ? await getMenuBarItem(part)
            : await getMenuItemInMenu(subMenu, part);
        if (menuItem) {
          if (i === numParts - 1) {
            return menuItem;
          } else {
    ...
    export async function getMenuItemInMenu(
      parentMenu: ElementHandle<Element>,
      label: string
    ): Promise<ElementHandle<Element> | null> {
      const items = await parentMenu.$$(
        `xpath=./ul/li[./div[text()="${label}" and ${xpContainsClass(
          'lm-Menu-itemLabel'
        )}]]`
      );
      if (items.length > 1) {
    ...
    export async function isAnyOpen(): Promise<boolean> {
      return (await context.page.$('.lm-Menu')) !== null;
    }

    export async function isOpen(path: string): Promise<boolean> {
        ...
        if (menuItem) {
          if (i === 0) {
            ...
            await page.waitForFunction(
              ({ menuCount, menuItem }) => {
                return (
                  document.querySelectorAll('.lm-Menu').length === menuCount &&
                  menuItem.classList.contains('lm-mod-active')
                );
              },
    ...
    export async function getOpenMenu(): Promise<ElementHandle<Element> | null> {
      const openMenus = await context.page.$$(
        '.lm-Widget.lm-Menu .lm-Menu-content'
      );
    ...
    export async function clickMenuItem(path: string): Promise<void> {
      const parts = path.split('>');
      const numParts = parts.length;
  ...
  export namespace activity {
    ...
    export async function isTabActive(name: string): Promise<boolean> {
      const tab = await getTab(name);
      return (
        tab &&
        (await tab.evaluate(tab => tab.classList.contains('lm-mod-current')))
      );
    }
    ...
    export async function activateTab(name: string): Promise<boolean> {
      const tab = await getTab(name);
      if (tab) {
        await tab.click();
  ...
  export namespace contents {
    export async function getContentMetadata(dirPath: string): Promise<any> {
    ...
    export async function fileExists(filePath: string): Promise<boolean> {
      ...
      return (
        content && (content.type === 'notebook' || content.type === 'file')
      );
    }
    ...
    export async function moveDirectoryToServer(
      sourcePath: string,
    ...
    export async function moveFileToServer(
      sourcePath: string,
      destinationPath?: string
    ): Promise<boolean> {
      const apiUrl = `${context.jlabBaseUrl}/api/contents`;
  ...
  export namespace filebrowser {
    export function xpBuildFileSelector(fileName: string): string {
      return `//div[@id='filebrowser']//li[./span[${xpContainsClass(
        'jp-DirListing-itemText'
      )} and ./span[text()="${fileName}"]]]`;
    }
    export function xpBuildDirectorySelector(dirName: string): string {
      return `//div[@id='filebrowser']//li[@data-isdir='true' and ./span[${xpContainsClass(
        'jp-DirListing-itemText'
      )} and ./span[text()="${dirName}"]]]`;
    }

    export async function revealFileInBrowser(filePath: string): Promise<void> {
    ...
    export async function isFileListedInBrowser(
      fileName: string
    ): Promise<boolean> {
      const item = await context.page.$(
        `xpath=${xpBuildFileSelector(fileName)}`
      );
      return item !== null;
    }
    ...
    export async function openHomeDirectory(): Promise<boolean> {
      const homeButton = await context.page.$(
        '.jp-FileBrowser .jp-FileBrowser-crumbs span'
      );
      if (!homeButton) {
        return false;
      ...
      await context.page.waitForFunction(() => {
        ...
        return (
          spans.length === 2 &&
          spans[0].classList.contains('jp-BreadCrumbs-home')
        );
      });
    ...
    export async function openDirectory(dirPath: string): Promise<boolean> {
    ...
    export async function refresh(): Promise<void> {
      ...
      const item = await page.$(
        `xpath=//div[@id='filebrowser']//button[${xpContainsClass(
    ...
    async function _openDirectory(dirName: string): Promise<boolean> {
      const item = await context.page.$(
        `xpath=${xpBuildDirectorySelector(dirName)}`
      );
      if (item === null) {
        return false;
      }
  ...
  export namespace notebook {
    ...
    export async function isAnyActive(): Promise<boolean> {
      return (await getNotebookInPanel()) !== null;
    }

    export async function open(name: string): Promise<boolean> {
      const fileItem = await context.page.$(
        `xpath=${filebrowser.xpBuildFileSelector(name)}`
      );
      if (fileItem) {
        fileItem.click({ clickCount: 2 });
        await context.page.waitForSelector(xpBuildActivityTabSelector(name), {
          state: 'visible'
        });
      }
    ...
    export async function getNotebookInPanel(
      name?: string
    ): Promise<ElementHandle<Element> | null> {
    ...
    export async function getToolbarItemByIndex(
      itemIndex: number,
      notebookName?: string
    ): Promise<ElementHandle<Element> | null> {
      if (itemIndex === -1) {
    ...
    export async function getToolbarItem(
      itemId: NotebookToolbarItemId,
      notebookName?: string
    ): Promise<ElementHandle<Element> | null> {
      const toolbar = await getToolbar(notebookName);

      if (toolbar) {
        const itemIndex = await context.page.evaluate(
          async (itemId: string) => {
        ...
        return getToolbarItemByIndex(itemIndex);
      }

      return null;
    }

    export async function clickToolbarItem(
      itemId: NotebookToolbarItemId,
      notebookName?: string
    ): Promise<boolean> {
    ...
    export async function revertChanges(): Promise<boolean> {
      if (!(await isAnyActive())) {
        return false;
      }
    ...
    export async function runCellByCell(
      ...
      if (callback) {
        ...
        await context.page.exposeFunction(
    ...
    export async function waitForRun(): Promise<void> {
      await context.page.evaluate(async () => {
        await window.galataip.waitForNotebookRun();
      });
    }

    export async function close(revertChanges = true): Promise<boolean> {
      if (!(await isAnyActive())) {
        return false;
      }
    ...
    export async function getCellInput(
      cellIndex: number
    ): Promise<ElementHandle<Element> | null> {
    ...
    export async function getCellInputExpander(
      cellIndex: number
    ): Promise<ElementHandle<Element> | null> {
      const cell = await getCell(cellIndex);
      if (!cell) {
    ...
    export async function isCellInputExpanded(
      cellIndex: number
    ): Promise<boolean> {
      const cell = await getCell(cellIndex);
      if (!cell) {
    ...
    export async function expandCellInput(
      cellIndex: number,
      expand: boolean
    ): Promise<boolean> {
    ...
    export async function getCellOutputExpander(
      cellIndex: number
    ): Promise<ElementHandle<Element> | null> {
      const cell = await getCell(cellIndex);
      if (!cell) {
    ...
    export async function isCellOutputExpanded(
      cellIndex: number
    ): Promise<boolean> {
      const cell = await getCell(cellIndex);
      if (!cell) {
    ...
    export async function expandCellOutput(
      cellIndex: number,
      expand: boolean
    ): Promise<boolean> {
    ...
    export async function getCellTextOutput(
      cellIndex: number
    ): Promise<string[] | null> {
    ...
    export async function isCellInEditingMode(
      cellIndex: number
    ): Promise<boolean> {
      const cell = await getCell(cellIndex);
      if (!cell) {
    ...
    export async function enterCellEditingMode(
      cellIndex: number
    ): Promise<boolean> {
      const cell = await getCell(cellIndex);
      if (!cell) {
    ...
    export async function leaveCellEditingMode(
      cellIndex: number
    ): Promise<boolean> {
    ...
    export async function isCellSelected(cellIndex: number): Promise<boolean> {
      return await context.page.evaluate((cellIndex: number) => {
        return window.galataip.isNotebookCellSelected(cellIndex);
      }, cellIndex);
    }

    export async function deleteCells(): Promise<boolean> {
      if (!(await isAnyActive())) {
        return false;
      }
    ...
    export async function setCell(
      ...
      if (
        !(await isCellSelected(cellIndex)) &&
        !(await selectCells(cellIndex))
      ) {
        return false;
      }
    ...
    export async function getCellType(
      cellIndex: number
    ): Promise<nbformat.CellType | null> {
      const notebook = await getNotebookInPanel();
      if (!notebook) {
        return null;
      }
    ...
    export async function createNew(name?: string): Promise<boolean> {
  ...
  export namespace statusbar {
    ...
    export async function show(): Promise<void> {
      ...
      await context.page.waitForSelector('#jp-main-statusbar', {
        state: 'visible'
      });
    }
  ...
  export namespace sidebar {
    ...
    export async function isTabOpen(id: SidebarTabId): Promise<boolean> {
      const tabButton = await context.page.$(
        `${buildTabSelector(id)}.lm-mod-current`
      );
      return tabButton !== null;
    }

    export async function getTabPosition(
      id: SidebarTabId
    ): Promise<SidebarPosition | null> {
      return await context.page.evaluate(
        async ({ tabSelector }) => {
    ...
    export async function moveTabToLeft(id: SidebarTabId): Promise<void> {
      await setTabPosition(id, 'left');
    }

    export async function moveTabToRight(id: SidebarTabId): Promise<void> {
      await setTabPosition(id, 'right');
    }

    export async function setTabPosition(
      id: SidebarTabId,
    ...
    export async function toggleTabPosition(id: SidebarTabId): Promise<void> {
      ...
      const switchMenuItem = await context.page.waitForSelector(
    ...
    export async function moveAllTabsToLeft(): Promise<void> {
      await context.page.evaluate(
        async ({ pluginId }) => {
    ...
    export async function openTab(id: SidebarTabId): Promise<void> {
    ...
    export async function getContentPanel(
      side: SidebarPosition = 'left'
    ): Promise<ElementHandle<Element>> {
      return await context.page.$(
    ...
    export async function open(side: SidebarPosition = 'left'): Promise<void> {
      const isOpen = await sidebar.isOpen(side);
      if (isOpen) {
        return;
      }

      await menu.clickMenuItem(
        `View>Show ${side === 'left' ? 'Left' : 'Right'} Sidebar`
      );
    ...
    export async function close(side: SidebarPosition = 'left'): Promise<void> {
      const isOpen = await sidebar.isOpen(side);
      if (!isOpen) {
        return;
      }
    ...
    export function buildTabSelector(id: SidebarTabId): string {
      return `.lm-TabBar.jp-SideBar .lm-TabBar-content li.lm-TabBar-tab[data-id="${id}"]`;
    }

    async function _waitForTabActivate(
  ...
  export async function isInSimpleMode(): Promise<boolean> {
  ...
  export async function toggleSimpleMode(simple: boolean): Promise<boolean> {
    const toggle = await context.page.$(
      '#jp-single-document-mode button.jp-switch'
    );
    const checked = (await toggle.getAttribute('aria-checked')) === 'true';
