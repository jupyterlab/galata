...
describe('Notebook Run', () => {
  ...
  test('Upload files', async () => {
    ...
    expect(
      await galata.contents.fileExists('uploaded/example.ipynb')
    ).toBeTruthy();
    expect(
      await galata.contents.fileExists('uploaded/jupyter.png')
    ).toBeTruthy();
  });
  ...
  test('Open directory uploaded', async () => {
    ...
    expect(
      await galata.filebrowser.isFileListedInBrowser('example.ipynb')
    ).toBeTruthy();
  });
