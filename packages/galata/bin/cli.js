...
const configFileData = fs.existsSync('./galata-config.json')
  ? fs.readFileSync('./galata-config.json')
  : undefined;
    ...
    flags: {
      ...
      include: {
        ...
        default: config.include
          ? JSON.stringify(config.include).replace(/"/g, '')
          : '[]'
      },
      exclude: {
        type: 'string',
        default: config.exclude
          ? JSON.stringify(config.exclude).replace(/"/g, '')
          : '[]'
      },
...
function getLatestRunTest() {
  ...
  testDirectories = testDirectories.sort((lhs, rhs) =>
...
function generateHTMLReport(testId) {
  ...
  if (fs.existsSync(galataOutputPath)) {
    ...
    if (galataOutput) {
      ...
      testsBasePath = path
        .normalize(path.join(testsBasePath, '/'))
        .replace(/\\/g, '\\\\');
  ...
  const template = fs.readFileSync(
    path.resolve(__dirname, '../report.ejs'),
    'utf-8'
  );
...
function runUpdateReferenceFiles() {
  ...
  console.log(
    `Copying test output files from ${testOutputDir} into reference directory ${referenceDir}`
  );
  ...
  let screenshots = fs.readdirSync(screenshotsOutputDir, {
    withFileTypes: true
  });
  screenshots = screenshots.filter(
    item => item.isFile() && item.name.endsWith('.png')
  );
  const numScreenshots = screenshots.length;
  if (numScreenshots > 0) {
    screenshots.forEach(screenshot => {
      fs.ensureDirSync(
        path.join(referenceScreenshotsDir, path.dirname(screenshot.name))
      );
      fs.copyFileSync(
        path.join(screenshotsOutputDir, screenshot.name),
        path.join(referenceScreenshotsDir, path.basename(screenshot.name))
      );
    });
  }
  ...
  htmlFiles = htmlFiles.filter(
    item => item.isFile() && item.name.endsWith('.html')
  );
  const numHtmlFiles = htmlFiles.length;
  if (numHtmlFiles > 0) {
    htmlFiles.forEach(htmlFile => {
      fs.ensureDirSync(
        path.join(referenceHtmlDir, path.dirname(htmlFile.name))
      );
      fs.copyFileSync(
        path.join(htmlOutputDir, htmlFile.name),
        path.join(referenceHtmlDir, path.basename(htmlFile.name))
      );
    });
  }
...
function getCLIScriptPath(scriptName, scriptDir, maxDepth = 10) {
  ...
  if (fs.existsSync(scriptPath)) {
    ...
    if (maxDepth === 0) {
      ...
      const scriptDirUnderGalata = path
        .join(selfDir, '../node_modules', scriptName, 'bin')
        .normalize();
      if (fs.existsSync(scriptDirUnderGalata)) {
        ...
        const jsExecutable = path.join(
          scriptDirUnderGalata,
          `${scriptName}.js`
        );
...
function runTests() {
  ...
  if (fs.existsSync(testOutputDir)) {
    log('info', `Deleting existing test output directory '${testId}'`, {
      save: false
    });
  ...
  fs.mkdirSync(`${testOutputDir}/reference-output/screenshots`, {
    recursive: true
  });
  ...
  const run = spawn.sync(
    jestPath,
    [...cli.input, ...jestDefaults, ...flagsArray],
    {
      stdio: [process.stdin, process.stdout, process.stderr]
    }
  );
